...
class LostFinderModule(BaseProcessModule):
    ...
    def process(message):
        ...
        try:
            ...
            elif message.getSubType() == 1 and LostFinderModule.is_report(message) and (
            context == LostFinderModule.CONTEXT3 | | context == LostFinderModule.CONTEXT4):
            ...
            # 消除一条记录
            elif message.getSubType() == 1 and LostFinderModule.is_delete(
            message) and context == LostFinderModule.CONTEXT5):
            ...
            if result.isLost == True:
                result = session.query(LostRecordModal).filter(id == recordId,
                                                               lostPersonQQ == message.getPersonQQ()).first()
            if not result:
            ...
            result = session.query(LostRecordModal).filter(id == recordId,
                                                           pickPersonQQ == message.getPersonQQ()).first()
            if not result:
                message.setContent(u"这条不是你发布的哦～")
            send(message, True)
            else:
            ...
            elif message.getSubType() == 1 and LostFinderModule.is_modify(
                message) and context == LostFinderModule.CONTEXT5):
            ...
            elif message.getSubType() == 1 and message.getContent() == u"取消" and (
                context == LostFinderModule.CONTEXT1 | | context == LostFinderModule.CONTEXT2):
            redisConnection.hset(REDIS_CONTEXT_CACHE_HASH_NAME, message.get_context_str(), "")
